import React, { useState, useEffect } from 'react';
import { Scale, X, Sparkles, Trophy, Star, RefreshCw, Package } from 'lucide-react';
import axiosClient from '../../api/axiosClient';

export default function ProductComparisonModal({
  isOpen,
  onClose,
  initialProductId = null,
  products = null,
}) {
  const [productsList, setProductsList] = useState([]);
  const [prodId1, setProdId1] = useState('');
  const [prodId2, setProdId2] = useState('');
  const [comparison, setComparison] = useState(null);
  const [loading, setLoading] = useState(false);
  const [listLoading, setListLoading] = useState(false);
  const [error, setError] = useState(null);

  // Normalize any API shape into a plain product array
  const extractList = (res) => {
    // axiosClient already returns ApiResponse body as `res`
    const payload = res?.data ?? res;
    if (Array.isArray(payload)) return payload;
    if (Array.isArray(payload?.content)) return payload.content;
    if (Array.isArray(payload?.data)) return payload.data;
    if (Array.isArray(payload?.data?.content)) return payload.data.content;
    if (Array.isArray(res?.content)) return res.content;
    return [];
  };

  const applyList = (list) => {
    const content = Array.isArray(list) ? list.filter(Boolean) : [];
    setProductsList(content);

    if (content.length === 0) {
      setError('No products available to compare.');
      setProdId1('');
      setProdId2('');
      return;
    }

    const initial =
      initialProductId != null
        ? content.find((p) => String(p.id) === String(initialProductId))
        : null;

    if (initial) {
      setProdId1(String(initial.id));
      const other = content.find((p) => String(p.id) !== String(initial.id));
      setProdId2(other ? String(other.id) : '');
    } else if (content.length >= 2) {
      setProdId1(String(content[0].id));
      setProdId2(String(content[1].id));
    } else {
      setProdId1(String(content[0].id));
      setProdId2('');
    }
  };

  useEffect(() => {
    if (!isOpen) return;

    setComparison(null);
    setError(null);

    // 1) Prefer products already shown on the page
    if (Array.isArray(products) && products.length > 0) {
      applyList(products);
      setListLoading(false);
      return;
    }

    // 2) Otherwise fetch (try /search then /products)
    setListLoading(true);

    const load = async () => {
      try {
        let res;
        try {
          res = await axiosClient.get('/search?page=0&size=50&sortBy=newest');
        } catch {
          res = await axiosClient.get('/products?page=0&size=50');
        }
        applyList(extractList(res));
      } catch (err) {
        console.error('Failed to load products for comparison', err);
        setError(err?.message || 'Failed to load products.');
        setProductsList([]);
      } finally {
        setListLoading(false);
      }
    };

    load();
  }, [isOpen, initialProductId, products]);

  if (!isOpen) return null;

  const handleCompare = async () => {
    if (!prodId1 || !prodId2 || String(prodId1) === String(prodId2)) {
      setError('Please select two different products.');
      return;
    }

    setLoading(true);
    setError(null);
    setComparison(null);

    try {
      const response = await axiosClient.post('/ai/vision/compare', {
        productId1: prodId1,
        productId2: prodId2,
      });

      // ApiResponse → payload in .data
      const data = response?.data ?? response;

      if (!data?.product1 || !data?.product2) {
        throw new Error('Invalid comparison response from server.');
      }

      setComparison(data);
    } catch (err) {
      console.error('Failed to execute AI product comparison', err);
      setError(
        err?.message ||
          err?.error ||
          'Comparison failed. Please try again or pick different products.'
      );
    } finally {
      setLoading(false);
    }
  };

  const formatPrice = (val) => {
    if (val == null || val === '') return '—';
    const n = Number(val);
    return Number.isFinite(n) ? `$${n.toFixed(2)}` : String(val);
  };

  const formatRating = (rating, reviewCount) => {
    const r = rating != null && rating !== '' ? Number(rating) : null;
    const c = reviewCount != null ? Number(reviewCount) : null;
    if (r == null) return 'No ratings yet';
    return `★ ${r.toFixed(1)}${c != null ? ` (${c})` : ''}`;
  };

  const isWinner = (productId) =>
    comparison?.winnerProductId &&
    String(comparison.winnerProductId) === String(productId);

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
      <div className="glass-panel w-full max-w-3xl p-6 rounded-3xl space-y-6 border border-slate-800 relative bg-slate-950/95 text-slate-100 max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-nexus-600 to-indigo-500 flex items-center justify-center text-white shadow-md">
              <Scale className="w-5 h-5" />
            </div>
            <div>
              <h3 className="text-base font-extrabold text-white flex items-center gap-1.5">
                AI Side-by-Side Product Comparison{' '}
                <Sparkles className="w-4 h-4 text-amber-400" />
              </h3>
              <p className="text-xs text-slate-400">
                Compare specs, ratings, price and AI pros/cons across two products.
              </p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 text-slate-400 hover:text-white rounded-xl hover:bg-slate-800"
            aria-label="Close"
          >
            <X className="w-5 h-5" />
          </button>
        </div>

        {/* Product Selectors */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
          <div className="space-y-1">
            <label className="font-bold text-slate-300">Product A</label>
            <select
              value={prodId1}
              onChange={(e) => setProdId1(e.target.value)}
              disabled={listLoading || productsList.length === 0}
              className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-nexus-500 disabled:opacity-50"
            >
              {listLoading && <option value="">Loading products…</option>}
              {!listLoading && productsList.length === 0 && (
                <option value="">No products found</option>
              )}
              {productsList.map((p) => (
                <option key={String(p.id)} value={String(p.id)}>
                  {p.name} ({formatPrice(p.price)})
                </option>
              ))}
            </select>
          </div>

          <div className="space-y-1">
            <label className="font-bold text-slate-300">Product B</label>
            <select
              value={prodId2}
              onChange={(e) => setProdId2(e.target.value)}
              disabled={listLoading || productsList.length === 0}
              className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-nexus-500 disabled:opacity-50"
            >
              {listLoading && <option value="">Loading products…</option>}
              {!listLoading && productsList.length === 0 && (
                <option value="">No products found</option>
              )}
              {productsList.map((p) => (
                <option key={String(p.id)} value={String(p.id)}>
                  {p.name} ({formatPrice(p.price)})
                </option>
              ))}
            </select>
          </div>
        </div>

        {error && (
          <div className="px-3 py-2 rounded-xl bg-rose-950/50 border border-rose-800 text-rose-300 text-xs">
            {error}
          </div>
        )}

        <button
          onClick={handleCompare}
          disabled={
            loading ||
            listLoading ||
            !prodId1 ||
            !prodId2 ||
            String(prodId1) === String(prodId2)
          }
          className="w-full py-2.5 bg-nexus-600 hover:bg-nexus-500 disabled:opacity-50 text-white rounded-xl text-xs font-extrabold flex items-center justify-center gap-2 shadow-lg transition"
        >
          {loading ? (
            <RefreshCw className="w-4 h-4 animate-spin" />
          ) : (
            <Scale className="w-4 h-4" />
          )}
          {loading ? 'Analyzing Product Matrix…' : 'Compare Products Side-by-Side'}
        </button>

        {/* Results */}
        {comparison && (
          <div className="space-y-6 border-t border-slate-800 pt-4 text-xs">
            <div className="p-4 bg-nexus-950/60 border border-nexus-800/80 rounded-2xl space-y-1">
              <div className="flex items-center gap-2 text-amber-400 font-bold">
                <Trophy className="w-4 h-4" /> AI Winner & Recommendation Summary
              </div>
              <p className="text-slate-200 leading-relaxed">
                {comparison.aiSummaryRecommendation || 'No summary available.'}
              </p>
            </div>

            <div className="overflow-x-auto rounded-2xl border border-slate-800">
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-900 text-slate-400 uppercase font-semibold border-b border-slate-800">
                  <tr>
                    <th className="p-3">Specification</th>
                    <th className="p-3 font-bold text-white">
                      <span className="flex items-center gap-1.5">
                        {comparison.product1?.name}
                        {isWinner(comparison.product1?.id) && (
                          <Trophy className="w-3.5 h-3.5 text-amber-400" />
                        )}
                      </span>
                    </th>
                    <th className="p-3 font-bold text-white">
                      <span className="flex items-center gap-1.5">
                        {comparison.product2?.name}
                        {isWinner(comparison.product2?.id) && (
                          <Trophy className="w-3.5 h-3.5 text-amber-400" />
                        )}
                      </span>
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-800">
                  <tr className="hover:bg-slate-900/50">
                    <td className="p-3 text-slate-400 font-semibold">Price</td>
                    <td className="p-3 font-bold text-emerald-400">
                      {formatPrice(comparison.product1?.price)}
                    </td>
                    <td className="p-3 font-bold text-emerald-400">
                      {formatPrice(comparison.product2?.price)}
                    </td>
                  </tr>
                  <tr className="hover:bg-slate-900/50">
                    <td className="p-3 text-slate-400 font-semibold">Rating</td>
                    <td className="p-3 text-amber-400 font-bold">
                      {formatRating(
                        comparison.product1?.rating,
                        comparison.product1?.reviewCount
                      )}
                    </td>
                    <td className="p-3 text-amber-400 font-bold">
                      {formatRating(
                        comparison.product2?.rating,
                        comparison.product2?.reviewCount
                      )}
                    </td>
                  </tr>
                  <tr className="hover:bg-slate-900/50">
                    <td className="p-3 text-slate-400 font-semibold">Category</td>
                    <td className="p-3">{comparison.product1?.categoryName || '—'}</td>
                    <td className="p-3">{comparison.product2?.categoryName || '—'}</td>
                  </tr>
                  <tr className="hover:bg-slate-900/50">
                    <td className="p-3 text-slate-400 font-semibold">In Stock</td>
                    <td className="p-3">
                      {comparison.product1?.stockQuantity != null
                        ? `${comparison.product1.stockQuantity} units`
                        : '—'}
                    </td>
                    <td className="p-3">
                      {comparison.product2?.stockQuantity != null
                        ? `${comparison.product2.stockQuantity} units`
                        : '—'}
                    </td>
                  </tr>
                </tbody>
              </table>
            </div>

            {comparison.specComparisonMatrix &&
              Object.keys(comparison.specComparisonMatrix).length > 0 && (
                <div className="space-y-2">
                  <h4 className="font-bold text-slate-300 flex items-center gap-1.5">
                    <Package className="w-3.5 h-3.5" /> Spec Matrix
                  </h4>
                  <div className="grid gap-1.5">
                    {Object.entries(comparison.specComparisonMatrix).map(
                      ([key, value]) => (
                        <div
                          key={key}
                          className="flex justify-between gap-4 px-3 py-2 rounded-xl bg-slate-900/60 border border-slate-800"
                        >
                          <span className="text-slate-400 font-semibold">{key}</span>
                          <span className="text-slate-200 text-right">{value}</span>
                        </div>
                      )
                    )}
                  </div>
                </div>
              )}

            {(comparison.product1Pros?.length > 0 ||
              comparison.product2Pros?.length > 0) && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-3 rounded-2xl bg-slate-900/50 border border-slate-800 space-y-2">
                  <div className="font-bold text-white flex items-center gap-1.5">
                    <Star className="w-3.5 h-3.5 text-amber-400" />
                    {comparison.product1?.name} – Highlights
                  </div>
                  <ul className="space-y-1 text-slate-300">
                    {(comparison.product1Pros || []).map((pro, i) => (
                      <li key={i} className="flex gap-1.5">
                        <span className="text-emerald-400">•</span> {pro}
                      </li>
                    ))}
                  </ul>
                </div>
                <div className="p-3 rounded-2xl bg-slate-900/50 border border-slate-800 space-y-2">
                  <div className="font-bold text-white flex items-center gap-1.5">
                    <Star className="w-3.5 h-3.5 text-amber-400" />
                    {comparison.product2?.name} – Highlights
                  </div>
                  <ul className="space-y-1 text-slate-300">
                    {(comparison.product2Pros || []).map((pro, i) => (
                      <li key={i} className="flex gap-1.5">
                        <span className="text-emerald-400">•</span> {pro}
                      </li>
                    ))}
                  </ul>
                </div>
              </div>
            )}
          </div>
        )}
      </div>
    </div>
  );
}