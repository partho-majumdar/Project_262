...
export default function RewardsPage() {
  ...
  const membershipTier =
    couponsData?.membershipTier != null && couponsData.membershipTier !== ''
      ? couponsData.membershipTier
      : null;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      <div className="glass-panel p-6 sm:p-10 rounded-3xl relative overflow-hidden space-y-3">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-nexus-950 border border-nexus-500/30 text-nexus-400 text-xs font-semibold">
          <Gift className="w-3.5 h-3.5 text-amber-400" /> Loyalty & Rewards
        </div>
        <h1 className="text-2xl sm:text-4xl font-extrabold text-white tracking-tight">
          Coupons & <span className="text-nexus-400">Rewards</span>
        </h1>
        <p className="text-xs sm:text-sm text-slate-300">
          View points, tier, cashback, and available promo codes from your account.
        </p>
      </div>
      ...
      <div className="space-y-4">
        ...
        {loading ? (
          <div className="glass-card p-8 rounded-3xl text-center space-y-2 animate-pulse">
            <Gift className="w-6 h-6 text-nexus-500 mx-auto" />
          ...
          <div className="glass-card p-8 rounded-3xl text-center border border-slate-800">
            <p className="text-xs text-slate-400">No active coupons right now.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {availableCoupons.map((coupon) => (
              <div
                ...
                className="glass-card p-6 rounded-3xl border border-slate-800 space-y-4 hover:border-slate-700 transition flex flex-col justify-between"
              >
                <div className="space-y-2">
                  <div className="flex justify-between items-center gap-2">
                    ...
                    {coupon.expiryDate && (
                      <span className="text-[10px] text-amber-400 font-mono font-bold flex items-center gap-1 shrink-0">
                ...
                >
                  {copiedCode === coupon.code ? (
                    ...
                    <Copy className="w-4 h-4 text-nexus-400" />
                  )}
